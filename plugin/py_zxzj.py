...
class Spider(Spider):  # 元类 默认的元类 type
	...
	def categoryContent(self,tid,pg,filter,extend):
		...
		suffix = '-'.join(params)
		url = 'https://zxzjhd.com/vodshow/{0}.html'.format(suffix)
		rsp = self.fetch(url)
		root = self.html(rsp.text)
		aList = root.xpath("//div[@class='stui-vodlist__box']/a")
		videos = []
	...
	def detailContent(self,array):
		tid = array[0]
		url = 'https://zxzjhd.com/detail/{0}.html'.format(tid)
		rsp = self.fetch(url)
		root = self.html(rsp.text)
	...
	def searchContent(self,key,quick):		
		url = 'https://zxzjhd.com/index.php/ajax/suggest?mid=1&wd={0}'.format(key)
		# getHeader()
		rsp = self.fetch(url)
	...
	config = {
		"player": {
			"dpp": {
				"sh": "DP播放",
				"pu": "https://jx.zxzjhd.com/dplayer.php?url=",
				"sn": 1,
				"or": 999
			}
		},
	...
	def playerContent(self,flag,id,vipFlags):
		result = {}
		url = 'https://zxzjhd.com/video/{0}.html'.format(id)
		rsp = self.fetch(url)
		root = self.html(rsp.text)
		...
		scriptUrl = 'https://zxzjhd.com/static/player/{0}.js'.format(jo['from'])
